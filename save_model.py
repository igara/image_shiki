...
for dir in traindir:
    ...
    for file in os.listdir(dir1):
        if file != ".DS_Store":
            ...
            filepath = dir1 + "/" + file
            # 画像を25x25pixelに変換し、1要素が[R,G,B]3要素を含む配列の128x128の２次元配列として読み込む。
...
# モデルを生成してニューラルネットを構築
model = Sequential()
model.add(Conv2D(32, (3, 3), padding="same", input_shape=(3, 128, 128)))
...
embedding_layer_names = set(layer.name
    for layer in model.layers if layer.name.startswith('dense_'))
...
# 学習を実行。10%はテストに使用。
model.fit(image_list, Y, epochs=10, batch_size=100, validation_split=0.1, callbacks=[tbcb])
